...
func Test_LoadAuditTest(t *testing.T) {
	fm := utils.NewKFolder()
	folder, err2 := utils.GetBenchmarkFolder("openshift", "v1.0.0", fm)
	...
	err = startup.SaveBenchmarkFilesIfNotExist("openshift", "v1.0.0", bFiles)
	if err != nil {
		t.Fatal(err)
	}
...
func Test_executeTests(t *testing.T) {
	...
	kb := OpenshiftAudit{ResultProcessor: GetResultProcessingFunction([]string{}), PlChan: plChan, CompletedChan: completedChan, Evaluator: evalcmd}
