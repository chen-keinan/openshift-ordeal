...
func TestRunAuditTests(t *testing.T) {
	tests := []struct {
		...
		{name: "Test_MultiCommandParams_OK", testFile: "CheckMultiParamOK.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: true},
		{name: "Test_MultiCommandParams_OK_With_IN", testFile: "CheckMultiParamOKWithIN.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: true},
		{name: "Test_MultiCommandParams_NOKWith_IN", testFile: "CheckMultiParamNOKWithIN.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: false},
		{name: "Test_MultiCommandParamsPass1stResultToNext", testFile: "CheckMultiParamPass1stResultToNext.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: false},
		{name: "Test_MultiCommandParamsComplex", testFile: "CheckMultiParamComplex.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: true},
		{name: "Test_MultiCommandParamsComplexOppositeEmptyReturn", testFile: "CheckInClauseOppositeEmptyReturn.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: false},
		{name: "Test_MultiCommandParamsComplexOppositeWithNumber", testFile: "CheckInClauseOppositeWithNum.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: false},
		{name: "Test_MultiCommand4_2_13", testFile: "CheckInClause4.2.13.yml", completedChan: make(chan bool), plChan: make(chan m2.OpenshiftAuditResults), wantTestSucceeded: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			kb := OpenshiftAudit{Evaluator: evalCmd, ResultProcessor: GetResultProcessingFunction([]string{}), PlChan: tt.plChan, CompletedChan: tt.completedChan}
...
//Test_NewopenshiftAudit test
func Test_NewopenshiftAudit(t *testing.T) {
	args := []string{"a", "i=1.2.3"}
	completedChan := make(chan bool)
	plChan := make(chan m2.OpenshiftAuditResults)
	evaluator := eval.NewEvalCmd()
	ka := NewopenshiftAudit(args, plChan, completedChan, []utils.FilesInfo{}, evaluator)
...
//Test_openshiftSynopsis test
func Test_openshiftSynopsis(t *testing.T) {
	args := []string{"a", "i=1.2.3"}
	completedChan := make(chan bool)
	plChan := make(chan m2.OpenshiftAuditResults)
	evaluator := eval.NewEvalCmd()
...
func Test_sendResultToPlugin(t *testing.T) {
	pChan := make(chan m2.OpenshiftAuditResults)
