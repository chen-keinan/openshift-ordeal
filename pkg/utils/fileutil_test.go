...
func Test_CreateBenchmarkFolderIfNotExist(t *testing.T) {
	fm := NewKFolder()
	err := CreateBenchmarkFolderIfNotExist("openshift", "v1.0.0", fm)
	assert.NoError(t, err)
	folder, err := GetBenchmarkFolder("openshift", "v1.0.0", fm)
	assert.NoError(t, err)
...
//Test_GetopenshiftBenchAuditFiles test
func Test_GetopenshiftBenchAuditFiles(t *testing.T) {
	fm := NewKFolder()
	err := CreateHomeFolderIfNotExist(fm)
	if err != nil {
		t.Fatal(err)
	...
	f, err := GetopenshiftBenchAuditFiles("openshift", "v1.0.0", fm)
	if err != nil {
		t.Fatal(err)
	}
	folder, err := GetBenchmarkFolder("openshift", "v1.0.0", fm)
...
//Test_GetopenshiftBenchAuditNoFolder test
func Test_GetopenshiftBenchAuditNoFolder(t *testing.T) {
	fm := NewKFolder()
	_, err := GetopenshiftBenchAuditFiles("openshift", "v1.1.0", fm)
...
func TestCreateBenchmarkFoldersErrorHomeFolder(t *testing.T) {
	...
	assert.Error(t, err)
	fmr := NewKFolder()
	path, err := GetBenchmarkFolder("openshift", "v1.0.0", fmr)
	assert.NoError(t, err)
	rhfp := GetHomeFolder()
	...
	err = CreateBenchmarkFolderIfNotExist("openshift", "v1.0.0", fm2)
	assert.Error(t, err)
}
...
func TestGetBenchmarkFoldersErrorHomeFolder(t *testing.T) {
	...
	fm := mocks.NewMockFolderMgr(ctl)
	fm.EXPECT().GetHomeFolder().Return("homePath", fmt.Errorf("error")).Times(1)
	_, err := GetBenchmarkFolder("openshift", "v1.0.0", fm)
	assert.Error(t, err)
}
